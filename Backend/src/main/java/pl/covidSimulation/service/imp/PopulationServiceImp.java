...
public class PopulationServiceImp implements PopulationService {
    ...
    public void createSimulationData(SimulationData simulationData){
        ...
        createAllDay(days,daysToDead,daysToHeal,newInfection,newDeaths,newRecoveries,simulationData,covidSimulationDataList);
    ...
    public void createAllDay(Integer days, Integer daysToDead, Integer daysToHeal, Integer[] newInfection, Integer[] newDeaths, Integer[] newRecoveries, SimulationData simulationData, List<Population> covidSimulationDataList){
        for(int i=1;i<days;i++){
            Population currentDate = new Population(covidSimulationDataList.get(i-1));
            if(daysToDead<=i){
                newDeadPeople(daysToDead,i,newInfection,newDeaths,simulationData,currentDate);
            }
            if(daysToHeal<=i) {
                newRecoveredPeople(newInfection,i,daysToHeal,simulationData,newRecoveries,currentDate);
            }
    ...
    public void newDeadPeople(Integer daysToDead, Integer i, Integer[] newInfection, Integer[] newDeaths, SimulationData simulationData, Population currentDate){
    ...
    public void newRecoveredPeople(Integer[] newInfection, Integer i, Integer daysToHeal, SimulationData simulationData, Integer[] newRecoveries, Population currentDate){
